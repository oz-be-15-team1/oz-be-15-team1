...
class Analyzer:
    ...
    def create_dataframe(self, transactions):
        """거래 내역으로부터 Pandas DataFrame을 생성합니다."""
        data = []
    ...
    def analyze_total_expense(self, df: pd.DataFrame, start_date, end_date):
        """총 지출 분석 및 시각화"""
    ...
    def analyze_total_income(self, df: pd.DataFrame, start_date, end_date):
        """총 수입 분석 및 시각화"""
    ...
    def analyze_category_expense(self, df: pd.DataFrame, start_date, end_date):
        """카테고리별 지출 분석 및 시각화"""
        expense_df = df[df["direction"] == "expense"]
    ...
    def analyze_account_balance(self, df: pd.DataFrame, start_date, end_date):
        """계좌 잔액 분석 및 시각화"""
        # 각 계좌별 최신 잔액 추출
        latest_balances = (
    ...
    def save_plot_image(self, plt, filename):
        """플롯을 이미지 파일로 저장합니다."""
    ...
    def run_analysis(self, analysis_type, period_type, start_date, end_date):
        ...
        # 분석 실행
        plt, description = analysis_methods[analysis_type](df, start_date, end_date)

        # 이미지 저장
        timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
        filename = f"{analysis_type}_{period_type}_{timestamp}.png"
        image_path = self.save_plot_image(plt, filename)
