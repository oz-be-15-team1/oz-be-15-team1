...
class CategoryRepository:
    ...
    def list_alive(user_id: int) -> QuerySet[Category]:
        ...
        return (
            Category.objects.filter(user_id=user_id, deleted_at__isnull=True)
    ...
    def list_deleted(user_id: int) -> QuerySet[Category]:
        ...
        return (
            Category.objects.filter(user_id=user_id, deleted_at__isnull=False)
            .only(*CategoryRepository._LIST_ONLY_FIELDS)
            .order_by("-deleted_at")
        )
    ...
    def soft_delete(user_id: int, category_id: int) -> int:
        ...
        return Category.objects.filter(
            id=category_id, user_id=user_id, deleted_at__isnull=True
    ...
    def restore(user_id: int, category_id: int) -> int:
        # 복구: UPDATE 1번
        return Category.objects.filter(
            id=category_id, user_id=user_id, deleted_at__isnull=False
