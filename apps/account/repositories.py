...
class AccountRepository:
    ...
    def get_active_accounts_only_fields(user):
        ...
        return Account.objects.filter(user=user, is_active=True).only(
            "id", "name", "source_type", "balance"
        )
    ...
    def get_accounts_summary_values(user):
        ...
        return Account.objects.filter(user=user).values("id", "name", "balance", "source_type")
